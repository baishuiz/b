Air.Module('B.directive.Repeater', function(require:any) {
 ...
 Array.prototype.unshift = function(_unshift){
  ...
  function getTemplateStr(str:string, idx:any, dataPath:string, dataPrefix:string) {
  ...
  function fixSelectElement(placeholder:any, target:any) {
   ...
   *返回：Repeater 实例
   **/
  function Repeater(template:any, currentScopeIndex:string, scopeStructure:any, parseTemplate:any) {
    ...
    function generatePlaceholder(target:any) {
    ...
    var getCount = function(path:string) {
    ...
    var addUI = function(num:number) {
      ...
      let isSelect = containerTagName === 'select';
      let initValue:any;
    ...
    var getPreviousElement = function(elm:HTMLElement) {
    ...
    var removeUI = function(num:number) {
      num = Math.abs(num);
      for (var i = 0; i < num; i++) {
    ...
    var updateUI = function(path:string) {
     ...
     *返回：undefind
     **/
    function bindRepeatData(repeater:any, dataPath:string, isinit?:boolean) {
      var activePath = '';
      ...
      for (var i = 0; i < pathNodes.length; i++) {
        ...
        if (isinit) {
          beacon(scope).on('updateRepeatData' + (pathNodes.slice(0, i + 1).join('')), function(e:any, args:any){
    ...
    function fixUnshift(val:any, value:any, descriptor:any){
      value.unshift =  function(item:any){
     ...
     *返回：repeat 数据源的描述符
     **/
    function createRepeatDataDescriptor(repeater:any, value:any, dataPath:string) {
      var oldLength = 0;
      value = value || [];
      var descriptor = {
        enumerable: true,
        ...
        set: function(val:any, isSub:boolean) {
